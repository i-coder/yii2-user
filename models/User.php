...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    const WORK_FOUR = 4;   // женщина
    const WORK_FIVE = 5;   // женщина

    const TYPE_ONE = 0;   // мастер
    const TYPE_TWO = 1;   // клиент

    const MASTER = 'master';   // мастер
    const CLIENT = 'client';   // клиент
    const ADMIN = 'administrator';   // клиент
    ...
    // названиях
    const EXT = '.jpg';
    public $userPhotoPath = '/img/users';
    ...
    //узнаем роль
    public static function getRulName($rol,$id=null){
        if($id){
            return array_key_exists($rol,\Yii::$app->authManager->getRolesByUser($id));
        }else{
            return array_key_exists($rol,\Yii::$app->authManager->getRolesByUser(Yii::$app->user->id));
        }
    }
    ...
    public static function getWorkArray()
    {
        return [
    ...
    {
        return [
            self::TYPE_ONE=>'Мастер',
            self::TYPE_TWO=>'Клиент',
        ];
    }
    ...
    public function rules()
    {
        return [
            ...
            ['work', 'in', 'range' => array_keys(self::getWorkArray())],  // Пол только из гендерного списка
            ['email', 'email'], // Формат электронной почты
            ['filename', 'string'], // Формат электронной почты
    ...
    public function attributeLabels()
    {
        return [
            ...
            'work' => Yii::t('user', 'С кем работаете?'),
            'type' => Yii::t('user', 'Вы'),
...
//            die();
            ...
            if (!file_exists(Yii::getAlias('@webroot').$path)) {
                mkdir(Yii::getAlias('@webroot').$path, 0755, true);   // Создаем директорию при отсутствии
            }
            if (is_object($this->photo)) {
              ...
              Image::thumbnail($this->photo->tempName, 200, 200)->save($this->image);
